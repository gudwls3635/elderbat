...
public class Man2 : MonoBehaviour
{
    ...
    string[] npcSelectItem = new string[] { "혹시 이건가요? (2초)", "그만둔다 (0초)", "" };
    ...
    public void selectBtn(int currentNpc, int index)
    {
        ...
        // 선택지에 따른 행동 보상
        if (npcSelectItem[index].Contains("혹시"))
        {
            if(PlayerScript.instance.getItemIndex("wedding") == -1) // 웨딩반지가 없을 때
            {
                GameManager.instance.interationScreen.transform.GetChild(4).GetComponent<Text>().text = "없잖아요? 장난치지 마세요!";
                transform.GetComponent<Animator>().SetInteger("igb", 2);

            }
            else // 웨딩반지가 있을 때
            {
                PlayerScript.instance.itemPop(PlayerScript.instance.getItemIndex("wedding"));
                GameManager.instance.interationScreen.transform.GetChild(4).GetComponent<Text>().text = "감사합니다! 정말 감사합니다!";
    ...
    void checkClick()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            else if (state == 3) // 선택지 누른 후 결과창 보여준 후
            {
                ...
                transform.GetComponent<Man2>().enabled = false;
            }
        }
    }
}
