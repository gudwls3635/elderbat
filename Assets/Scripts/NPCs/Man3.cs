...
public class Man3 : MonoBehaviour
{
    ...
    string[] npcSelectItem = new string[] { "기타를 건넨다 (4초)", "그만둔다 (0초)", "" };
    ...
    public void selectBtn(int currentNpc, int index)
    {
        ...
        // 선택지에 따른 행동 보상
        if (npcSelectItem[index].Contains("기타"))
        {
            if (PlayerScript.instance.getItemIndex("gitar") == -1) // 웨딩반지가 없을 때
            {
                GameManager.instance.interationScreen.transform.GetChild(4).GetComponent<Text>().text = "없어? 뭐 괜찮아. 혹시라도 준다면 반드시 보답하지!";
                transform.GetComponent<Animator>().SetInteger("igb", 2);

            }
            else // 웨딩반지가 있을 때
            {
                PlayerScript.instance.itemPop(PlayerScript.instance.getItemIndex("gitar"));
                GameManager.instance.interationScreen.transform.GetChild(4).GetComponent<Text>().text = "정말이야? 고마워!! 이제 나도 명가수다!";
    ...
    void checkClick()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            else if (state == 3) // 선택지 누른 후 결과창 보여준 후
            {
                ...
                transform.GetComponent<Man3>().enabled = false;
            }
        }
    }
