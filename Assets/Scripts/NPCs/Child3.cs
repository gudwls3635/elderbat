...
public class Child3 : MonoBehaviour
{
    // Start is called before the first frame update
    int state, count;
    int[,] npcSelectPercent = new int[,] { { 100, -1, -1 }, { 100, -1, -1 }, { -1, -1, -1 } };
    string[] npcSelectItem = new string[] { "미니카를 건넨다 (2초)", "그만둔다 (0초)", "" };
    ...
    public void selectBtn(int currentNpc, int index)
    {
        ...
        // 선택지에 따른 행동 보상
        if (npcSelectItem[index].Contains("미니카"))
        {
            PlayerScript.instance.itemPop(PlayerScript.instance.getItemIndex("minicar"));
            GameManager.instance.interationScreen.transform.GetChild(4).GetComponent<Text>().text = "아저씨 감사합니다!" ;
            // 선택지를 누른 결과 시간 감소
        ...
        if (Input.GetMouseButtonDown(0))
        {
            if (state == 1) // npc와 interation 성공 후 선택지 띄우기
            {
                if(PlayerScript.instance.getItemIndex("minicar") == -1) //미니카 없을 때
                {
                    ...
                    transform.GetComponent<Child3>().enabled = false;
                }
                else //미니카 있을 때
                {
                    state = 2;
