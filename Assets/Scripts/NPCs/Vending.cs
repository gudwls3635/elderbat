...
public class Vending : MonoBehaviour
{
    ...
    int state,count;
    int[,] npcSelectPercent = new int[,] { { 40, 80, 100 }, { 100, -1, -1 }, { -1, -1, -1 } };
    string[] npcSelectItem = new string[] { "뒤져본다 (3초)", "그만둔다 (0초)",""};
    int[,] npcReward= new int[,] { {0,100,500 },{ 0,0,0},{0,0,0 } };
    int npcTime = 3;
    ...
    public void initData()
    {
        ...
        transform.GetComponent<SpriteRenderer>().sprite = imgList[rand];
    }
    ...
    public void selectBtn(int currentNpc,int index)
    {
        ...
        {
            GameManager.instance.money += npcReward[index, rewardIndex] * 1000;
